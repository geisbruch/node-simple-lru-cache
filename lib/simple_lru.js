...
Cache.prototype.get = function(key,hit){
    ...
    /*
    * Define if maxTime is used and if it's valid.
    */
    var cacheTimeValid = (cacheVal && cacheVal.time == undefined) || 
                         (cacheVal && Date.now() - cacheVal.time < this.maxTime);

    if(cacheVal && hit && cacheTimeValid) {
        this.hit(cacheVal)
    } else {
        return undefined
    }
